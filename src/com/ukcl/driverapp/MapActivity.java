...
import com.ukcl.driverapp.adapter.DrawerAdapter;
import com.ukcl.driverapp.base.ActionBarBaseActivitiy;
import com.ukcl.driverapp.db.DBHelper;
import com.ukcl.driverapp.fragment.ClientRequestFragment;
import com.ukcl.driverapp.fragment.FeedbackFrament;
import com.ukcl.driverapp.fragment.JobFragment;
import com.ukcl.driverapp.model.ApplicationPages;
import com.ukcl.driverapp.model.Bill;
import com.ukcl.driverapp.model.RequestDetail;
import com.ukcl.driverapp.model.RequestInfoModel;
...
import com.ukcl.driverapp.widget.MyFontTextViewDrawer;
...
*/
		...
		*/
		...
		if(user.getPicture() != null){
			...
			ImageLoader.getInstance().displayImage(user.getPicture(), ivMenuProfile);
		}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		if (position == 0) {
			...
			//startActivity(new Intent(this, MyBookingsActivity.class));
			startActivity(new Intent(this, SurgeActivity.class));
		} 
		/*else if (position == 4) {
		}*/
	...
	public void getRequestDetails() {
		...
		AndyUtils.showCustomProgressDialog(this, "",
				...
				false);
		HashMap<String, String> map = new HashMap<String, String>();
		map.put(AndyConstants.URL,AndyConstants.ServiceType.REQUEST_DETAILS);	
		map.put(AndyConstants.Params.REQUEST_ID, preferenceHelper.getRequestId()+"");
		new HttpRequester(this, map,
				AndyConstants.ServiceCode.REQUEST_DETAILS, this);
	}
	...
	public void onTaskCompleted(String response, int serviceCode) {
		...
		case AndyConstants.ServiceCode.LOGIN:
			...
			break;
		case AndyConstants.ServiceCode.REQUEST_DETAILS:
			AndyUtils.removeCustomProgressDialog();
			Gson gson = new Gson();
			RequestInfoModel requestInfoModel = gson.fromJson(response,
					RequestInfoModel.class);
			if (requestInfoModel != null) {
				if (requestInfoModel.getResponse().getRequest().getLater().equals("1")) {
					if (requestInfoModel.getResponse().getRequest().getIs_started().equalsIgnoreCase("1")) {
						checkRequestStatus();
					}else {
						loadHomeScreen(response);
					}
				} else {
					checkRequestStatus();
				}
		...
		case AndyConstants.ServiceCode.APPLICATION_PAGES:
			...
			arrayListApplicationPages.remove(2);
			ApplicationPages applicationPagesBooking = new ApplicationPages();
			applicationPagesBooking.setData("");
			applicationPagesBooking.setId(3);
			applicationPagesBooking.setIcon("");
			applicationPagesBooking.setTitle("My Bookings");
			arrayListApplicationPages.add(applicationPagesBooking);
			
			ApplicationPages applicationPagesSurgeArea = new ApplicationPages();
			applicationPagesSurgeArea.setData("");
			applicationPagesSurgeArea.setId(4);
			applicationPagesSurgeArea.setIcon("");
			applicationPagesSurgeArea.setTitle("Surge Areas");
			arrayListApplicationPages.add(applicationPagesSurgeArea );
	...
	private void loadHomeScreen(String response) {
		/*if (!parseContent.isSuccess(response)) {
			if (parseContent.getErrorCode(response) == AndyConstants.REQUEST_ID_NOT_FOUND) {
				preferenceHelper.clearRequestData();
				AndyUtils.removeCustomProgressDialog();
	...
	private void changeState() {
		...
		map.put(AndyConstants.Params.IS_LOGGING_OUT, "1");
