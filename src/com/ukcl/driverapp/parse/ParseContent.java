package com.ukcl.driverapp.parse;
...
import com.ukcl.driverapp.maputills.PolyLineUtils;
import com.ukcl.driverapp.model.ApplicationPages;
import com.ukcl.driverapp.model.BeanRoute;
import com.ukcl.driverapp.model.BeanStep;
...
import com.ukcl.driverapp.model.VehicalType;
...
import com.ukcl.driverapp.utills.ReadFiles;
	...
	public Bill parsebillwhenwalkcomplete(String response){
		...
		try {
			...
			if (jsonObject.getBoolean(KEY_SUCCESS)) {
				...
				bill.setService_fee(billobject.getString("service_fee"));
				bill.setDriver_fee(billobject.getInt("driver_fee"));
			...
			if (jsonObject.getBoolean(KEY_SUCCESS)) {
				...
				if (jsonArray.length() > 0) {
					...
					if (object.getInt(AndyConstants.Params.REQUEST_ID) != AndyConstants.NO_REQUEST) {
						...
						try {
							requestDetail.setE_latitude(ownerDetailObject
									.getString(AndyConstants.Params.DEST_2_LATITUDE));
							requestDetail.setE_longitude(ownerDetailObject
									.getString(AndyConstants.Params.DEST_2_LONGITUDE));
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
			...
			if (jsonObject.getBoolean(KEY_SUCCESS)) {
				...
				if (jsonArray.length() > 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						history.setLater(object
								.getString(AndyConstants.Params.IS_ADVANCE));
						history.setAdvance_type(object
								.getString(AndyConstants.Params.ADVANCE_TYPE));
